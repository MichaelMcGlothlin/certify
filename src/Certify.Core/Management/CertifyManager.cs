...
namespace Certify.Management {
 public class CertifyManager {
  ...
  public async Task<CertificateRequestResult> PerformCertificateRequest ( VaultManager vaultManager, ManagedSite managedSite ) {
   vaultManager = vaultManager ?? new VaultManager ( Properties.Settings.Default.VaultPath, ACMESharp.Vault.Providers.LocalDiskVault.VAULT );
   ...
   config.ChallengeType = config.ChallengeType ?? "http-01";
   ...
   foreach ( var domain in allDomains ) {
    ...
    var identifierAlreadyValid = false;
     ...
     if ( authorization.Identifier.Authorization.IsPending () ) {
      if ( managedSite.SiteType == ManagedSiteType.LocalIIS ) {
       ...
       if ( config.PerformExtensionlessAutoConfig && !authorization.ExtensionlessConfigCheckedOK ) {
        ...
        var identifierValidated = vaultManager.CompleteIdentifierValidationProcess ( authorization.Identifier.Alias );
   ...
   if ( allIdentifiersValidated ) {
    var primaryDnsIdentifier = identifierAuthorizations.First ().Identifier.Alias;
    var alternativeDnsIdentifiers = identifierAuthorizations.Where ( i => i.Identifier.Alias != primaryDnsIdentifier ).Select ( i => i.Identifier.Alias ).ToArray ();
  ...
  public async Task<List<CertificateRequestResult>> PerformRenewalAllManagedSites ( Boolean autoRenewalOnly = true ) {
   siteManager.LoadSettings ();
   ...
   foreach ( var s in sites.Where ( s => s.IncludeInAutoRenew == true ) ) {
    results.Add ( await PerformCertificateRequest ( vaultManager, s ) );
   }
