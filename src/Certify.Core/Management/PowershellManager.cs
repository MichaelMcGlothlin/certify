...
namespace Certify {
 public class PowershellManager {
  ...
  public static Int32 GetPowershellVersion () {
   try {
    var powershellVersion = 0;
    var regval = Registry.GetValue ( @"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\PowerShell\3\PowerShellEngine", @"PowerShellVersion", null ).ToString ();
    if ( regval != null ) {
     var ver = regval.Split ( '.' );
     powershellVersion = Int32.Parse ( ver[ 0 ] );
    }
  ...
  public Boolean IsAcmeSharpModuleInstalled () {
   ps.Commands.Clear ();
   ...
   return res.IsOK && ( res.Result as IEnumerable )?.OfType<Object> ().Count () > 0;
  }
  ...
  private APIResult InvokeCurrentPSCommand ( Boolean discardErrors = false ) {
   try {
    var results = ps.Invoke ();
    return new APIResult { IsOK = true, Result = results };
   } catch ( AcmeClient.AcmeWebException awExp ) {
    if ( awExp.Response?.ProblemDetail != null ) {
    ...
    return new APIResult { IsOK = false, Message = ex.ToString (), Result = ex };
   }
  }
  ...
  public APIResult CompleteChallenge ( String identifierRef, String challengeType = "http-01", Boolean regenerate = true ) {
   ps.Commands.Clear ();
  ...
  public APIResult SubmitChallenge ( String identifierRef, String challengeType = "http-01" ) {
   ...
   var logAction = "Powershell: Submit-ACMEChallenge " + identifierRef + " -Challenge " + challengeType;
  ...
  public APIResult NewCertificate ( String identifierRef, String certAlias, String[] subjectAlternativeNameIdentifiers = null ) {
   ...
   String sanList = null;
   if ( subjectAlternativeNameIdentifiers?.Length > 0 ) {
  ...
  public APIResult ExportCertificate ( String certAlias, String vaultFolderPath, Boolean pfxOnly = false ) {
   var certKey = certAlias;
