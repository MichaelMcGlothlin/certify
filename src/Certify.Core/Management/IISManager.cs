...
namespace Certify.Management {
 ...
 public class IISManager {
  ...
  public Version GetIisVersion () {
   ...
   using ( var componentsKey = Registry.LocalMachine.OpenSubKey ( @"Software\Microsoft\InetStp", false ) ) {
    if ( componentsKey != null ) {
     var majorVersion = (Int32) componentsKey.GetValue ( "MajorVersion", -1 );
     var minorVersion = (Int32) componentsKey.GetValue ( "MinorVersion", -1 );
  ...
  private IEnumerable<Site> GetSites ( ServerManager iisManager, Boolean includeOnlyStartedSites ) => includeOnlyStartedSites
       ? iisManager.Sites.Where ( s => s.State == ObjectState.Started )
       : iisManager.Sites;

  public List<SiteBindingItem> GetSiteList ( Boolean includeOnlyStartedSites ) {
  ...
  private SiteBindingItem GetSiteBinding ( Site site, Binding binding ) => new SiteBindingItem () {
   SiteId = site.Id.ToString (),
   SiteName = site.Name,
   Host = binding.Host,
   PhysicalPath = site.Applications[ @"/" ].VirtualDirectories[ @"/" ].PhysicalPath,
   Port = binding.EndPoint.Port,
   IsHTTPS = String.Equals ( binding.Protocol, @"https", StringComparison.OrdinalIgnoreCase ),
   Protocol = binding.Protocol,
   HasCertificate = binding.CertificateHash != null
  };

  private Site GetSite ( String siteName, ServerManager iisManager ) => GetSites ( iisManager, false ).FirstOrDefault ( s => s.Name == siteName );
  ...
  public X509Certificate2 StoreCertificate ( String host, String pfxFile ) {
   ...
   var certificate = new X509Certificate2 ( pfxFile, String.Empty, X509KeyStorageFlags.MachineKeySet | X509KeyStorageFlags.PersistKeySet | X509KeyStorageFlags.Exportable ) {
    FriendlyName = host + @" - " + DateTime.Today.ToShortDateString ()
   };
  ...
  public void CleanupCertificateDuplicates ( X509Certificate2 certificate, String hostPrefix ) {
   if ( certificate.FriendlyName.Length < 10 ) {
    return;
   }
  ...
  public void InstallCertificateforBinding ( Site site, X509Certificate2 certificate, String host, Int32 sslPort = 443, Boolean useSNI = true ) {
   var store = new X509Store ( StoreName.My, StoreLocation.LocalMachine );
   store.Open ( OpenFlags.OpenExistingOnly | OpenFlags.ReadWrite );
   using ( var iisManager = new ServerManager () ) {
    var siteToUpdate = iisManager.Sites.FirstOrDefault ( s => s.Id == site.Id );
    if ( siteToUpdate != null ) {
     var internationalHost = _idnMapping.GetUnicode ( host );
  ...
  public Boolean InstallCertForDomain ( String host, String pfxPath, Boolean cleanupCertStore = true, Boolean skipBindings = false ) {
  ...
  private void DisableSSLViaRegistry ( String protocolKey ) {
   ...
   var SSLProtocolKey = GetRegistryBaseKey ( RegistryHive.LocalMachine ).OpenSubKey ( @"SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\" + protocolKey, true ) ?? SSLProtocolsKey.CreateSubKey ( protocolKey );

   //create key for protocol if it doesn't exist

   //create Client key if required
   var clientKey = SSLProtocolKey.OpenSubKey ( "Client", true ) ?? SSLProtocolKey.CreateSubKey ( "Client" );
   ...
   var serverKey = SSLProtocolKey.OpenSubKey ( "Server", true ) ?? SSLProtocolKey.CreateSubKey ( "Server" );
  ...
  private void DisableSSLCipherViaRegistry ( String cipher ) {
   ...
   var cipherKey = GetRegistryBaseKey ( RegistryHive.LocalMachine ).OpenSubKey ( @"SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Ciphers\" + cipher, true ) ?? cipherTypesKey.CreateSubKey ( cipher );
