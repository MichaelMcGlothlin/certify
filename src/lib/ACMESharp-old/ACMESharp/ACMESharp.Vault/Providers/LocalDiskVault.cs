...
namespace ACMESharp.Vault.Providers {
 public class LocalDiskVault : IVault {
  ...
  public Boolean CreatePath { get; set; }

  public Boolean BypassEFS { get; set; }

  public Boolean IsDisposed { get; private set; }

  public Boolean IsStorageOpen { get; private set; }
  ...
  public Boolean TestStorage () => Directory.Exists ( RootPath )
           && File.Exists ( _tagFile )
           && File.Exists ( _vaultFile );

  public void InitStorage ( Boolean force ) {
   AssertNotDisposed ();
  ...
  public void OpenStorage ( Boolean initOrOpen ) {
   AssertNotDisposed ();
  ...
  public VaultInfo LoadVault ( Boolean required ) {
   AssertOpen ();
  ...
  public void SaveVault ( VaultInfo vault ) {
   ...
   _vaultMeta = _vaultMeta ?? new EntityMeta<VaultInfo> {
    CreateDate = now,
    CreateUser = who,
  ...
  public IEnumerable<VaultAsset> ListAssets ( String nameRegex = null, params VaultAssetType[] type ) {
   ...
   if ( type?.Length == 0 ) {
    type = Enum.GetValues ( typeof ( VaultAssetType ) ).Cast<VaultAssetType> ().ToArray ();
   }
   ...
   foreach ( var vat in type ) {
    ...
    if ( Directory.Exists ( vatPath ) ) {
     ...
     assets.AddRange (
      vatFiles.Select (
       x => new FileVaultAsset ( x, Path.GetFileName ( x ), vat, File.GetAttributes ( x ).HasFlag ( FileAttributes.Encrypted ) )
      )
     );
    }
   }
  ...
  public VaultAsset CreateAsset ( VaultAssetType type, String name, Boolean isSensitive = false,
