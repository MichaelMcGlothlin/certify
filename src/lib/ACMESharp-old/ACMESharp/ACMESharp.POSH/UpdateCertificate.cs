  ...
  protected override void ProcessRecord () {
   using ( var vlt = Util.VaultHelper.GetVault ( VaultProfile ) ) {
    ...
    if ( NewAlias != null ) {
     ...
     ci.Alias = Equals ( NewAlias, String.Empty ) ? null : NewAlias;
    }
    ...
    if ( !LocalOnly ) {
     ...
     if ( ( Repeat || String.IsNullOrEmpty ( ci.CrtPemFile ) )
      ...
      crtDerAsset = crtDerAsset ?? vlt.CreateAsset ( VaultAssetType.CrtDer, crtDerFile );

      crtPemAsset = crtPemAsset ?? vlt.CreateAsset ( VaultAssetType.CrtPem, crtPemFile );
      ...
      if ( linksEnum != null ) {
       ...
       if ( upLink != null ) {
        ...
        try {
         ...
         v.IssuerCertificates = v.IssuerCertificates ?? new OrderedNameMap<IssuerCertificateInfo> ();
         ...
         if ( Repeat || !v.IssuerCertificates.ContainsKey ( sernum ) ) {
          ...
          if ( Repeat || issuerDerAsset == null ) {
           issuerDerAsset = issuerDerAsset ?? vlt.CreateAsset ( VaultAssetType.IssuerDer, cacertDerFile );
          ...
          if ( Repeat || issuerPemAsset == null ) {
           issuerPemAsset = issuerPemAsset ?? vlt.CreateAsset ( VaultAssetType.IssuerPem, cacertPemFile );
