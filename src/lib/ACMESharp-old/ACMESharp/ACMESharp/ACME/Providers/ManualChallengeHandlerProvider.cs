...
namespace ACMESharp.ACME.Providers {
 ...
 public class ManualChallengeHandlerProvider : IChallengeHandlerProvider {
  ...
  public System.Boolean IsSupported ( Challenge c ) => c is DnsChallenge || c is HttpChallenge;

  public IChallengeHandler GetHandler ( Challenge c, IReadOnlyDictionary<System.String, System.Object> initParams ) {
   ...
   if ( initParams?.Count > 0 ) {
    ...
    if ( initParams.ContainsKey ( WRITE_OUT_PATH.Name ) ) {
     p = (System.String) initParams[ WRITE_OUT_PATH.Name ];
    }

    if ( initParams.ContainsKey ( APPEND.Name ) ) {
     a = (System.Boolean) initParams[ APPEND.Name ];
    }

    if ( initParams.ContainsKey ( OVERWRITE.Name ) ) {
     o = (System.Boolean) initParams[ OVERWRITE.Name ];
    }
